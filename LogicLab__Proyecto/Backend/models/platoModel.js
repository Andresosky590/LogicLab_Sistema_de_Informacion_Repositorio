...
const PlatoModel = {
    ...
    findAll: (callback) => {
        const query = `
            SELECT id_Platos, NombrePlato, Precio, Descripcion, id_Categoria, Disponible, ImagenUrl
            FROM Platos
            ORDER BY id_Platos
        `;
    ...
    // Para saber la ImagenUrl actual de un plato antes de reemplazarla
    // o de borrar el plato (así se puede borrar el archivo viejo del disco).
    findById: (id, callback) => {
        db.query(
            `SELECT id_Platos, ImagenUrl FROM Platos WHERE id_Platos = ?`,
            [id],
            callback
        );
    },
    ...
    create: (datos, callback) => {
        const query = `
            ...
            VALUES (?, ?, ?, ?, ?)
        `;
        const disponible = datos.disponible === undefined
            ? 'Disponible'
            : (datos.disponible ? 'Disponible' : 'Agotado');
    ...
    updateDisponibilidad: (id, disponible, callback) => {
        const query = `
            ...
            WHERE id_Platos = ?
        `;
        db.query(query, [disponible ? 'Disponible' : 'Agotado', id], callback);
    },

    updateImagen: (id, imagenUrl, callback) => {
        const query = `
            UPDATE Platos
            SET ImagenUrl = ?
            WHERE id_Platos = ?
        `;
        db.query(query, [imagenUrl, id], callback);
    },
